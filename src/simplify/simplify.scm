...
(define (make-analyzer ->expression expression-> known-operators)
  (let ((auxiliary-variable-table
	 (make-hash-table 50))
	(reverse-table (make-hash-table 50))
	(uorder '())
	(priority '()))
...
;    (fluid-let ((incremental-simplifier #f))	; always #f
...
;	)
    ...
    (define (new-analysis)		
      (set! auxiliary-variable-table
	    (make-hash-table 50))
      (set! reverse-table (make-hash-table 50))
      (set! uorder '())
      (set! priority '())
      'done)
...

	...
	(let ((v (hashq-ref symbolic-operator-table
				 (operator sexpr)
				 #f)))
	  (if v
