...
(define (gcd-memoizer poly/gcd)
  (let ((table
	 (make-hash-table 50)))
	   ; use with unordered-poly-hash, unordered-pair-equal?
    (define (the-memoized-gcd p1 p2)
      (if *gcd-memoizer-enabled*
	  (let ((x (cons p1 p2)))
	    (let ((seen (hashx-ref unordered-poly-hash
				   unordered-pair-equal? table x *not-seen*)))
	      (if (not (eq? seen *not-seen*))
		  (begin (set! *gcd-hit* (fix:+ *gcd-hit* 1))
			 seen)
		  (let ((ans (poly/gcd p1 p2)))
		    (set! *gcd-miss* (fix:+ *gcd-miss* 1))
		    (hashx-set! unordered-poly-hash
				unordered-pair-equal? table x ans)
		    ans))))
	  (poly/gcd p1 p2)))
    the-memoized-gcd))
