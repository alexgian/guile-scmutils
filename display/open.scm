...
;;; interface to gnuplot for interactive plotting

; example:
; (define w (gnuplot-open))
; (gnuplot-function w  (lambda (x) (sin (* pi x))) -1 1 .01)

(use-modules (ice-9 popen))	; pipes
...
(define (graphics-clear win)
  (list-set! win 0 '())
  (list-set! win 1 '()))

(define (gnuplot-draw window)
  (let ((n (length (car window)))	;; number of functions
	(port (caddr window)))		;; pipe to gnuplot process
    (define (print-func-num n l)
      (if (pair? l)
	  (begin (display ", '-' with line title 'func " port)
		 (display n port)
		 (display "'" port)
		 (print-func-num (1+ n) (cdr l)))))
       
    (display "plot " port)
    (if (> n 0) 
	(begin (display "'-' with line title 'func 1'" port)
	       (print-func-num 2
			       (cdar window))))
    (if (> (length (cadr window)) 0)	;; scatter points
	(if (> n 0) 
	    (display ", '-' notitle with points pointtype 0" port)
	    (display "'-' notitle with points pointtype 0" port)))
    (newline port)			;; start inline data:
    (map (lambda (l)			;; for each function
	   (map (lambda (p)		;; plot each point
		  (display (car p) port)
		  (display " " port)
		  (display (cadr p) port)
		  (newline port))
		l)
	   (display "e\n" port))	;; end of inline data
	 (car window))
    (if (> (length (cadr window)) 0)	;; scatter points                        
	(begin (map (lambda (p)
		      (display (car p) port)
...
;; returns list of (x y) points for x = x0...x1
(define (walk-function f x0 x1 dx)
  (if (> x0 x1)
      '()
      (cons (list x0 (f x0))
	    (walk-function f (+ x0 dx) x1 dx))))

(define (gnuplot-function window f x0 x1 dx)
  (list-set! window 0
	(append  (car window)
		 (list (walk-function f x0 x1 dx))))
  (gnuplot-draw window))

(define (gnuplot-point window x y)
  (list-set! window 1
	     (append (cadr window)
		     (list (list x y))))
  (gnuplot-draw window))

(define (gnuplot-function-old window f x0 x1 dx)
  (if *gnuplotting* (newline *gnuplotting*))
  (display "plot '-' with line title 'function'\n" window)
  (let loop ((x x0) (fx (f x0)))
    ...
    (let ((nx (+ x dx)))
      (let ((nfx (f nx)))
	(begin (write x window)
	       (display " " window)
	       (write fx window)
	       (newline window))
	(if (< (* (- nx x0) (- nx x1)) 0.)
	    (loop nx nfx)))))
  (display "e\n" window)		; signals end of inline data
  (force-output window))
...
(define *frame-x-position* (if #t ;(eq? 'unix microcode-id/operating-system)
			       -10 532))
(define *frame-y-position* 0)
(define *frame-width* (if #t ;(eq? 'unix microcode-id/operating-system) 
			  400 100))
(define *frame-height* (if #t ;(eq? 'unix microcode-id/operating-system) 
			   400 100))
...
(define plot-point gnuplot-point)
...
(define plot-function gnuplot-function)
